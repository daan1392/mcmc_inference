...
def output_twinscatterpdf_plot(exp, prior, posterior, save_path):
    ...
    # 2. Create twin axes for the histograms
    # ax_histx shares the X-axis with ax, but has independent Y (for density)
    ax_histx = ax.twinx()
    # ax_histy shares the Y-axis with ax, but has independent X (for density)
    ax_histy = ax.twiny()

    # 3. Control Layering (Z-order)
    # We want 'ax' (scatter) on top, and histograms behind.
    # Matplotlib draws axes in creation order, so twins are usually on top.
    # We force 'ax' to the front and make its background transparent.
    ax.set_zorder(10)
    ...
    if exp.type == 'integral':
        ...
        ax.axhspan(
            ...
            facecolor='C0',
            alpha=0.2,
            zorder=9 # Slightly behind line
        )

    # Pre-calculate Y values
    ...
    ax.plot(
        prior, prior_y,
        color='C1', alpha=0.5, ls='None', marker='.',
        label='Prior Samples', zorder=10
    )

    ax.plot(
        posterior, post_y,
        color='C2', ls='None', alpha=0.5, marker='.',
        label='Posterior Samples', zorder=10
    )
