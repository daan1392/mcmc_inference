...
def get_kernel(kernel_type, n_features):
    ...
    if kernel_type == "RBF":
        ...
        structure = Matern(
            length_scale=[1.0] * n_features, nu=2.5, length_scale_bounds=(1e-5, 1e10)
        )
...
def train_surrogates(config_path):
    ...
    for exp in cfg["experiments"]:
        ...
        try:
            ...
            if "exp" in df_y.columns:
                y_vals = df_y["exp"].values
            elif "chi2" in df_y.columns:
                y_vals = df_y["chi2"].values
            else:
                y_vals = df_y.iloc[:, 0].values
