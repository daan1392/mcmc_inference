...
% Title and Author
\title{Using microscopic and integral measurements to infer nuclear data parameters}
\author{Daan Houben}
\date{\today}
...
Nuclear data is considered as the major source of uncertainty in some reactor observables, most notably effective multiplication factor (\keff). The nuclear data available in evaluated nuclear data libraries, such as JEFF-4.0, are a result of a complex fitting procedure including theoretical models, microscopic experiments, expert judgement and sometimes integral experiments. Although integral experiments are often not directly including in the fitting procedure due to the computational cost, they form an important step towards validating the nuclear data vector. In this work, a Bayesian Optimization framework is proposed which enables consolidating microscopic energy dependent measurements with integral experiments to estimate nuclear data parameters. Due to the computational cost of evaluating the performance of nuclear data in high-fidelity simulation software, most notably in integral experiments, Gaussian Processes (GPs) are trained. Subsequently, the GPs are used in a Markov Chain Monte Carlo (MCMC) algorithm in order to obtain a posterior estimate of the parameter(s) to infer. A case study is performed on the $\Gamma_\gamma$ width of \nuclide{53}{Cr} in the keV energy range using data recently published by P. Maroto et al. (2025) \cite{perez-maroto_50cr_2025}, complemented with integral experiments. 

\section{Background and mathematical motivation}

To enable efficient sampling of the parameter space, we replace the computationally expensive high-fidelity model $f(\bm{\theta})$ with a Gaussian Process (GP) emulator. A GP defines a distribution over functions, specified by a mean function $m(\bm{\theta})$ and a covariance kernel $k(\bm{\theta}, \bm{\theta}')$.
...
In this work, the GP is trained on a set of input-output pairs generated from the high-fidelity model. For any candidate $\bm{\theta}$ proposed during MCMC, the GP prediction $\mu_{GP}(\bm{\theta})$ is used as the model response.

\section{Markov Chain Monte Carlo (MCMC) Framework}
...
\begin{itemize}
    ...
    \item $P(D \mid \bm{\theta}) = \mathcal{L}(\bm{\theta})$ is the \textbf{Likelihood} (the goodness-of-fit).
    \item $P(\bm{\theta})$ is the \textbf{Prior} (initial beliefs about parameter bounds).
\end{itemize}

We employ the Metropolis-Hastings algorithm to sample from this posterior. The acceptance ratio $\alpha$ for a proposed step $\bm{\theta}_{new}$ is defined as:

\begin{equation}
    \alpha = \min\left(1, \frac{\mathcal{L}(\bm{\theta}_{new}) P(\bm{\theta}_{new})}{\mathcal{L}(\bm{\theta}_{old}) P(\bm{\theta}_{old})} \right)
\end{equation}

Crucially, the calculations operate in \textbf{log-space} to prevent numerical underflow. The acceptance criterion becomes a comparison of log-likelihoods ($ll$):

\begin{equation}
    \ln(\alpha) = \min(0, ll_{new} + \ln(P_{new}) - ll_{old} - \ln(P_{old}))
\end{equation}

\section{Likelihood Formulation}

The definition of the log-likelihood $\ln(\mathcal{L})$ depends on the nature of the experimental data. We distinguish between \textbf{Integral Experiments} (scalar values) and \textbf{Microscopic Experiments} (correlated vectors).

% --- FIXED SECTION BELOW ---
% Used \texorpdfstring to allow math in the body, but text in the PDF bookmarks
\subsection{Derivation: The Relationship Between \texorpdfstring{$\chi^2$}{Chi-Squared} and Log-Likelihood}

Standard Bayesian inference assumes that experimental errors follow a Gaussian (Normal) distribution. The probability density function (PDF) for a set of observations $D$ given prediction $M(\bm{\theta})$ is:
