...
Now the opposite is assumed and all microscopic measurement points are expected to be highly correlated. Then, we take the chi-squared per degree of freedom, so Eq. \ref{eq:likelihood} becomes
\begin{equation}
    \begin{split}
...
\section{Results}
\subsection{Chromium-53}
In Figure x, the validation of the Gaussian Process is shown for (a) the microscopic expeirment and (b) an integral experiment. It can be seen that the 
\section{Discussion}
