...
When performing a Bayesian Optimization, uncertainties of other nuclides should also be taken into account in order not to correct for bias induced by other sources. For criticality experiments, these other nuclear data uncertainties are estimated using relative first-order sensitivity coefficients calculated using Serpent-2. Here, sensitivity coefficients describing the effect of nuclear data perturbations on \keff{} in ECCO-33 multi-group energy structure are calculated using Generalized Perturbation Theory (GPT) \cite{aufieroCollisionHistorybasedApproach2015} for all nuclides in the system of consideration and for the reactions fission, capture, in/elastic scattering, neutron fission multiplicities and prompt neutron fission energy spectrum. The relative sensitivity vectors are included in a matrix , $S_{k,\sigma}$, for the integral experiments under consideration. Then using SANDY \cite{fioritoJEFF33CovarianceApplication2019}, wrapping the NJOY2016 code \cite{muirNJOYNuclearData}, covariance matrices $\Sigma_\sigma$ were generated for the nuclides and reactions under consideration in the same multi-group energy structure. The Sandwich formula \cite{broadheadSensitivityUncertaintyBasedCriticality2004} was used to propagate the covariance matrix from the nuclear data to the \keff{} variance. It is given by
\begin{equation}\label{eq:sandwich}
    \Sigma_k=S_{k,\sigma}^T \cdot \Sigma_\sigma \cdot S_{k,\sigma},
\end{equation}
in which $\Sigma_k$ is the covariance matrix on \keff{} for the nuclear systems under consideration. The nuclear data which is not being adjusted is not included in the covariance matrix or sensitivity profiles and therefore, only the remaining nuclear data uncertainties are propagated. The remaining variance is then added to the experiemental variance. This ensures the nuclear data adjustment is not correcting for bias in other nuclear data. However, it also makes the integral experiments less informative since they are heavily subjected to nuclear data coming from fuel materials. 
\section{Description of cases}
\subsection{Chromium-53}
Chromium is an isotope that is used frequently in nuclear reactors, more specifically in stainless steel in order to increase its corrosion resistance (11-26\%). This, together with the scattering and capture cross sections being significant, makes it an element important for criticality safety in some nuclear systems \cite{trkovSensitivitySelectedBenchmarks}. A major nuclide of chromium with a relatively poor estimation of the cross section is \nuclide{53}{Cr} in the 1-10 keV range. Several microscopic measurements exist which measure the capture yield, most notably Guber (2011) and Stieglitz (19xx). However, these experiments are not very consistent with the data available in evaluated nuclear data libraries. In a recent study by Perez-Maroto et al. (2025), new capture yield measurements were performed with a thin sample. It is seen as a possibility to observe whether integral experiments sensitive to Cr-53 in the 1-10 keV range can be used in a Bayesian framework to increase the confidence in the nuclear data of \nuclide{53}{Cr}. 

Several integral experiments were previously identified as candidates, see \cite{nobreNewlyEvaluatedNeutron2021}, however, in this paper the criticality experiments PMI-002 and HMI-001 were selected due to their significant sensitivity in the 1-10 keV range, as can be seen in Figure \ref{fig:sensint} in which the calculated sensitivity to Cr-53 capture is shown for PMI-002 and HMI-001. 
\begin{figure}
  \centering
...
Case    & PMI002    & HMI001        & ntof (ind.)   & mean  & r. unc. (\%)      \\ \hline
1       &           &               &               & 4.14  & 5.0               \\
...
In contrast, when the datapoints are considered independent from each other, the weight of the microscopic datapoints increases heavily. This can be seen in Fig. \ref{fig:mcmcall}.
\begin{figure}
\centering
\begin{subfigure}{.3\textwidth}
  \centering
